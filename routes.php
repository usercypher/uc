...
$app->groupRoute($group, '', 'pipe/{option?}/{class?}', array('pipe' => array('Pipe_Cli_Pipe'), 'ignore' => array('--global')));
...
$app->groupRoute($group, 'GET', 'home', array('pipe' => array('Pipe_Book_Home'))); // This route is for 'GET /home', triggers 'Pipe_Book_Home'
$app->groupRoute($group, 'GET', 'create', array('pipe' => array('Pipe_Book_Create'))); // This route is for 'GET /create', triggers 'Pipe_Book_Create'

// Define a route for editing a book, with an ID parameter (title_id and ID are extracted with regex)
$app->groupRoute($group, 'GET', 'edit/{title_id:([a-zA-Z0-9-]+)-([0-9]+)}', array('pipe' => array('Pipe_Book_Edit'))); // Route with a title_id and ID regex pattern for 'GET /edit/{title_id}-{id}'
$app->groupRoute($group, 'GET', 'edit/{id:[0-9]+}', array('pipe' => array('Pipe_Book_Edit'))); // Fallback route for 'GET /edit/{id}' where the ID is numeric
...
$app->groupRoute($group, 'POST', 'store', array('pipe' => array('Pipe_Book_Store'))); // 'POST /book/store' triggers 'Pipe_Book_Store'
$app->groupRoute($group, 'POST', 'update', array('pipe' => array('Pipe_Book_Update'))); // 'POST /book/update' triggers 'Pipe_Book_Update'
$app->groupRoute($group, 'POST', 'delete', array('pipe' => array('Pipe_Book_Delete'))); // 'POST /book/delete' triggers 'Pipe_Book_Delete'
